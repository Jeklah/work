...
from autolib.models.qxseries.system import System
...
from autolib.models.qxseries.generator_v2 import GeneratorV2
...
class Qx(DeviceController):
    ...
    def __init__(self, hostname: str, http_session: requests.Session = None):
        ...
        self._v2_base_url = f"http://{self._hostname}:8080/api/v2/"
        ...
        self._ip_net = IPNetworking(self._base_url, self.log, self._hostname, self._sfp, self, http_session)
        self._preset = PresetManager(self._base_url, self.log, self._hostname, http_session)
        self._nmos = NmosClient(self._base_url, self.log, self._hostname, self.type_name, http_session)
        self._loudness = Loudness(self._base_url, self.log, self._hostname, http_session)
        self._ptp = Ptp(self._base_url, self.log, self._hostname, http_session, self)
        self._generatorv2 = GeneratorV2(self._v2_base_url, self.log, self._hostname, http_session)
        self._system = System(self._base_url, self.log, self._hostname, http_session)
    ...
    @property
    def generator_v2(self) -> GeneratorV2:
        """
        Access the `Generator` methods and properties
        """
        return self._generatorv2

    @property
    def system(self) -> System:
        """\
        Access the system-wide configuration methods and properties
        """
        return self._system
    ...
    @staticmethod
    def _convert_version(major: int, minor: int, patch: int, build: int) -> int:
        """
    ...
    def request_capability(self, capability: DeviceCapability) -> bool:
        """
        ...
        Supported capabilities::
            ...
            GeneratorMode.SDI, GeneratorMode.2100, GeneratorMode.2022_6 - Configure the unit for generation of the specified type

            AnalyserMode.IP, AnalyserMode.SDI - Configure the unit for analysis of the specified type

        """
        match capability:
            case OperationMode():
                return self._request_operation_mode_capability(OperationMode(capability))
            case GeneratorOutput():
                return self._request_generator_mode_capability(GeneratorOutput(capability))
            case AnalyserInput():
                return self._request_analyser_source_capability(AnalyserInput(capability))
    ...
    def _request_generator_mode_capability(self, mode: GeneratorOutput) -> bool:
        ...
        """
        self.log.warning("GeneratorMode capability requested but on Qx only supports SDI generation.")
        return mode == GeneratorOutput.SDI

    def _request_analyser_source_capability(self, source: AnalyserInput) -> bool:
        """\
        Request that the device switch generator mode to that specified.
        """
        self.log.warning("AnalyserSource capability requested but on Qx, the analyser mode is set by the OperationMode capability.")
        return False
    ...
    def query_capability(self, capability) -> bool:
        ...
        match capability:
            case OperationMode():
                return self._query_operation_mode_capability(OperationMode(capability))
            case GeneratorOutput():
                return self._query_generator_mode_capability(GeneratorOutput(capability))
            case AnalyserInput():
                return self._query_analyser_source_capability(AnalyserInput(capability))
    ...
    def _query_operation_mode_capability(self, operation_mode: OperationMode) -> bool:
        ...
        """
        if operation_mode not in OperationMode:
            self.log.warning(f"Requested capability query ({operation_mode}) is invalid.")
            return False
        ...
        if version >= self.COMBINED_SDI_VERSION:
            if operation_mode in (OperationMode.SDI, OperationMode.SDI_STRESS) and current_op_mode in (OperationMode.SDI, OperationMode.COMBINED_SDI_QXL):
                return True
            elif operation_mode == current_op_mode == OperationMode.IP_2110:
                return True
            elif operation_mode == current_op_mode == OperationMode.IP_2022_6:
                return True
        else:
            if operation_mode in OperationMode:
                return current_op_mode == operation_mode
    ...
    def _query_generator_mode_capability(self, generator_mode: GeneratorOutput) -> bool:
        ...
        """
        return generator_mode == GeneratorOutput.SDI and self.operation_mode in (OperationMode.SDI, OperationMode.SDI_STRESS)
    ...
    def _query_analyser_source_capability(self, analyser_source: AnalyserInput) -> bool:
        ...
        """
        return (analyser_source == AnalyserInput.SDI and self.operation_mode in (OperationMode.SDI, OperationMode.SDI_STRESS)) or (analyser_source == AnalyserInput.IP and self.operation_mode in (OperationMode.IP_2110, OperationMode.IP_2022_6))
    ...
    def _rest_checks(self, retries: int = 10, delay: int = 10) -> bool:
        ...
        for retry in range(retries):
            try:
                # At the moment let's just try and get the system/about information via the property
                _ = self.about
                break
...
class TemporaryPreset:
    """\
    Context manager that will upload and activate a preset on a Qx / QxL at the start of scope and then remove it
...
class TemporaryDictPreset:
    """\
    Context manager that will upload a dictionary as a preset file to a device and activate it at the start of scope and
    then remove it at the end of scope for use in tests and test fixtures. This operates on a deep copy of the supplied
    dict.
    """
    def __init__(self, qx, preset_dict, preset_name):
        self._qx = qx
        self._preset_dict = deepcopy(preset_dict)
        self._preset_name = preset_name

    def __enter__(self):
        with tempfile.NamedTemporaryFile(mode="w+t") as temp_local_preset_file:
            json.dump(self._preset_dict, temp_local_preset_file)
            temp_local_preset_file.flush()
            self._qx.preset.upload(temp_local_preset_file.name, self._preset_name)
            self._qx.preset.load(self._preset_name)
        return self
...
class StateSnapshot:
    ...
    This can be really useful in test fixtures if the 'restore' argument is set to True. When this is set, the
    state snapshot is loaded on the device in __exit__ before it's removed from disk. This has the effect of ensuring
    that a unit is left in its previous state following a test. e.g.

        @pytest.fixture(scope='module')
        def qx(test_qx_hostname):
            qx = make_qx(test_qx_hostname)
            qx.request_capability(OperationMode.IP_2110)
            with StateSnapshot(qx, restore=True):
                qx.generator_v2.generator_bouncing_box = False
                qx.generator_v2.ident_enabled = False
                qx.nmos.enable()
                yield qx

    In this example the unit is switched to support 2110 then a snapshot is taken. Some configuration is then changed
    and the yield returns the Qx fixture. After the yield (at module teardown time in this case) the StateSnapshot
    context manager restores the state of the unit after the 2110 switch but before the other configuration changes.

    """

    def __init__(self, qx, restore=False):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self._restore:
            self.log.info(f"Restoring stored temporary StateSnapshot preset: {self._state_preset_temp_name}")
            self._qx.preset.load(self._state_preset_temp_name)
    ...
    @property
    def name(self):
        return self._state_preset_temp_name
