...
# Set up standard logging for test_system
...
def generator_qx(test_generator_hostname):
    """
    Creates generator Qx object. Tears down configuration after test.
...
def analyser_qx(test_analyser_hostname):
    """
    Creates analyser Qx object. Tears down configuration after test.
...
def test_aes_sdi(aes_config, generator_qx, analyser_qx):
    """
    [Happy] Tests AES channel, pairs and groups are set correctly in SDI mode.

    param: aes_config json object
    param: generator_qx qx object
    param: analyser_qx qx object [Unused currently]
    """
...
@pytest.mark.parametrize("aes_config", _generator_aes(), ids=_generator_aes_formatter)
def test_aes_2022_6(aes_config, generator_qx, analyser_qx):
    """
...
@pytest.mark.sdi
@pytest.mark.parametrize("aes_config", _sad_generator_aes(), ids=_generator_aes_formatter)
def test_aes_sdi_sad_inputs(aes_config, generator_qx, analyser_qx):
    """
    ...
    """
    generator_qx.request_capability(OperationMode.SDI)
    with pytest.raises(QxException, match='Failed to configure AES channel configuration'):
        _test_aes(aes_config, generator_qx)


@pytest.mark.ip2022_6
@pytest.mark.parametrize("aes_config", _sad_generator_aes(), ids=_generator_aes_formatter)
def test_aes_2022_6_sad_inputs(aes_config, generator_qx, analyser_qx):
    """
    [Sad] Tests AES channel, pairs and groups are set correctly in 2022-6 mode.
...
@pytest.mark.sdi
def test_aes_sdi_bad_inputs(generator_qx, analyser_qx):
    """
...
@pytest.mark.ip2022_6
def test_aes_2022_6_bad_inputs(generator_qx, analyser_qx):
    """
...
def _test_aes_bad(generator_qx):
    ...
    response = requests.put(f"{generator_qx.path}/aesIO/config", headers={'Content-Type': 'application/json'}, json=data)
    ...
    """
    response = requests.put(f"{generator_qx.path}/aesIO/config",
                            headers={'Content-Type': 'application/xml'},
                            data=textwrap.dedent(xml_data).encode())
    assert response.status_code == 415  # Should return Unsupported Media Type client error status

    data = b"VGhpcyBpcyBhIHJlYWxseSBsb25nIHN0cmluZyBvZiBhYnNvbHV0ZSBnYXJiYWdlIG1hc3F1ZXJhZGluZyBhcyBKU09OIGluIGEgYm"\
