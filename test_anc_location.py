...
def _generate_expected_s352_locations(qx_analyser):
    """
    ...
    parsed_analysed_standard = qx_analyser.analyser.parse_analyser_status(qx_analyser.analyser.get_analyser_status())

    resolution = parsed_analysed_standard["resolution"]
    scan_type = parsed_analysed_standard["frame_rate"]["unit"]
    link_count = parsed_analysed_standard["type"]["link_number"]
    data_rate = parsed_analysed_standard["type"]["data_rate_Gb"]
    standard_level = parsed_analysed_standard["type"]["level"]
    ...
    if standard_level == "A":
        ...
        if data_rate <= 3.0 and link_count == 1:
            ...
            sub_image_search = ["subImage1", "subImage2",
                                "linkBSubImage1", "linkBSubImage2"]
    ...
    if data_rate == 1.5 or standard_level == "B" or data_rate == 6.0 and link_count == 1:
        ...
        s352_exp_line = [10, 572] if standard_level == "B" else s352_locations[line_num][scan_type]
        # Set expected channel locations for 1.5G || level B standard || 6G single standards
        s352_exp_channel = ["yPos"]
    else:
        s352_exp_line = s352_locations[line_num][scan_type]
...
def test_s352_location_full(all_standards, generator_unit, analyser_unit):
    """
...
def _check_s352_location(standards_list, generator_qx, analyser_qx):
    """
    ...
    log.info(f'{generator_qx.hostname} - Configure generator to use current standard data - {standards_list}')
    ...
    for subimg, line, channel in _generate_expected_s352_locations(analyser_qx):
        ...
        analyser_qx.anc.setup_inspector(identifier=("custom", 1, 65), subimage=subimg, position=channel,
                                        range=("inside", line-1, line+1))
