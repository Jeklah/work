...
def test_required_method_docstring_get():
    ...
    assert exc.value.args[0].get("message", None) == "Method GET (GET returning dict from JSON) tuple requires a path / parameter template and a docstring"


def test_required_method_docstring_gettext():
    """\
    Try to create a class from the metaclass with a GETTEXT method (GET but return body as text string and encoding
    as a tuple) with no docstring
    """
    ...
    assert exc.value.args[0].get("message", None) == "Method GETTEXT (GET returning text / encoding tuple) tuple requires a path / parameter template and a docstring"
