...
class SdiAnalyser(APIWrapperBase,
    ...
    def get_analyser_status(self):
        ...
        """
        warnings.warn("New code should use `ParsedStandard(self.analyser_status.get('standard'))`. Do not use in new code.",
                      DeprecationWarning)
    ...
    def get_parsed_standard(self) -> ParsedStandard:
        """\
        Get the current analyser standard as a ParsedStandard object
        """
        return ParsedStandard(self.analyser_status.get('standard'))
    ...
    def get_link_and_subimage_crcs_dict(self) -> dict:
        ...
        """
        crc_detail_url = f"{self._base_url}/analyser/detail"
        crc_head = {'links': set(), 'link_subimages': set()}
        crc_leaf = crc_head
